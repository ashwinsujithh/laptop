(function (root, factory) {
  ...
  function getWarrantyLabel(item) {
    ...
    return warranty || "Confirm warranty";
  }

  function getAccessorySpecifications(item) {
  ...
  function validateInventory(items, type) {
    if (!Array.isArray(items)) {
      return [`${type} inventory must contain an array`];
    }
    ...
    items.forEach((item, index) => {
      ...
      if (type === "laptop" && item.brand && item.model) {
        ...
        if (originalPosition) {
          errors.push(
            `${label} duplicates the configuration from item ${originalPosition}`
          );
        } else {
  ...
  async function loadCatalogs(loadJson) {
